...
We used `estimate_L=TRUE`, to estimate the number of credible sets (`L`) for the merged regions. This information will be used later in the fine-mapping step. We could set `maxSNP` to limit the number of SNPs in the region, if merged regions contain too many SNPs,  
...
We can also run these steps without LD, using the functions `merge_region_data_noLD()`.
...
if (any(problematic_region_ids)) {
  rerun_region_data <- screen_res$selected_region_data[problematic_region_ids]

  finemap_rerun_region_res <- finemap_regions_noLD(rerun_region_data, 
                                                   group_prior = group_prior,
                                                   group_prior_var = group_prior_var)
}
```
