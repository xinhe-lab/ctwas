...
This post-processing procedure contains multiple steps, as described in details below. To automate these steps, we made functions `postprocess_region_merging()` and `postprocess_region_merging_noLD()`, which automatically perform these region merging steps with or without LD.
...
If you would like to run each step separately, please see the sections below.

### Select boundary genes
...
Similar to region merging, the post-processing procedure for LD-mismatch analysis also involves several steps. We provided a function `postprocess_LD_mismatch()`, which automatically performs the post-processing steps for LD-mismatch analysis.
...
In practice, it could be helpful to first run region merging procedure (with `postprocess_region_merging()`), and then perform the LD-mismatch analysis (with `postprocess_LD_mismatch()`) using the outputs from region merging. 
