...
Then we select the candidate regions with two options. 

Option 1. When `screen_method = "nonSNP_PIP"`, we select regions with the total PIP from 
molecular traits (non-SNPs) greater than 0.5 (by default). 
It returns `screened_region_data` which contains a subset of region data for selected regions.
It also returns the values of non-SNP PIPs for all the regions. We could check the non-SNP PIPs and further adjust the screened regions by choosing different `min_nonSNP_PIP` cutoffs.
...
Option 2. When `screen_method = "cs"`, we select regions with at least one credible set.
It returns `screened_region_data`, as well as estimated L (number of credible sets) for all the regions. Note: this option is not available for the "no-LD" version. 
...
```{r finemap_regions_L5, eval=FALSE}
...
If we run screening regions by the number of credible sets `screen_method = "cs"`, 
we could use the estimated L for each region:
```{r finemap_regions_estimated_L, eval=FALSE}
# directory for computed correlation matrices
cor_dir <- file.path(outputdir, "cor_matrix")
