...
It returns a vector of non-SNP PIPs for each region. 
We could further narrow down the candidate regions by using more stringent `min_nonSNP_PIP` cutoffs.
...
nonSNP_PIPs <- screen_regions_res$nonSNP_PIPs
```

Option 2. When `screen_method = "cs"`, we select regions with at least one credible set.
It returns a vector of estimated L (number of credible sets) for each region. 
Note: this option is not available in the "no-LD" version. 
...
res <- screen_regions(region_data,
                      ...
                      verbose = FALSE)
screened_region_data <- res$screened_region_data
estimated_L <- screen_regions_res$estimated_L
```
