...
Here we assume the `z_snp` (a data frame of GWAS summary statistics) is available. Please see the section "GWAS Z-score"  in the ["Preparing cTWAS input data"][preparing_input_data_tutorial] tutorial for details. 
...
We need `weights`, a list object containing preprocessed weights for the molecular traits. The weights can be in the PredictDB (default, used in this tutorial) or FUSION format. In this tutorial, we used the weights from GTEx liver eQTL data from PredictDB. We provide a function `preprocess_weight()` to harmonize the prediction models and LD reference. 
...
+ `susie_alpha_res`: a data frame with finemapping results of molecular traits and the single effect probabilities (alpha) in all credible sets. This will be used later when computing combined gene PIPs. 
