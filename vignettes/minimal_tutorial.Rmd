...
The "no-LD" version of cTWAS requires several input datasets: (i) GWAS summary statistics; (ii) prediction models (called "weights" in our documentation) and the reference data, including information about all the variants; and (iii) the definition of the genomic regions. We describe below the steps for data preparation and preprocessing before running cTWAS. More details about data preparation, including preparing the LD data, which is required for full cTWAS, are given in the tutorial, "Preparing cTWAS input data".
...
Additionally, we require `ref_snp_info` as a “reference” of the variants. `ref_snp_info` is a data frame, containing the positions and allele information of all the variants from the entire genome. We provide a function, `create_snp_map()` to process `region_info` to the correct format and map variants from the reference to regions, which takes input of `region_info` and `ref_snp_info`, and returns `region_info` and `snp_map`.
...
We need a list `weights` which contains preprocessed weights. The weights can be in the PredictDB (default, used in this tutorial) or FUSION format. We provide a function `preprocess_weight()` to harmonize the prediction models and LD reference. 
...
We use the function `ctwas_sumstats_noLD()` to run cTWAS analysis without LD. It takes as input preprocessed GWAS z-scores (`z_snp`), `weights`, `region_info`, `snp_map`, and will perform the main steps: (i) computing Z-scores of molecular traits; (i) estimating model parameters; (iii) screening regions potentially containing causal molecular traits, and (iv) fine-mapping those regions. Note that cTWAS uses a strategy to speed up computation. It initially estimates parameters using a fraction of SNPs (by default, we use `thin = 0.1`, i.e. 10% of the SNPs) - step (ii), and select regions likely containing signals - step (iii). After that, it includes all SNPs in the selected regions to do full fine-mapping analysis in step (iv). 
