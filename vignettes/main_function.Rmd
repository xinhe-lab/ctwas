...
We use the function `ctwas_sumstats()` to run cTWAS analysis with LD. It takes as input preprocessed GWAS z-scores (`z_snp`), and prediction models of molecular traits (`weights`). In addition, it requires the reference data including information about regions (`region_info`), reference variant list (`snp_map`) and reference LD (`LD_map`). The function performs the main steps: (i) computing Z-scores of molecular traits; (i) estimating model parameters; (iii) screening regions potentially containing causal molecular traits, and (iv) fine-mapping those regions to discover causal molecular traits. 
