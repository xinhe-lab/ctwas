...
The `preprocess_z_snp()` function harmonizes GWAS z-scores and the reference data based on the included allele information. It will remove variants not included in the reference data. 
...
We limit variants in weights, reference (`snp_map`) and GWAS SNPs (`z_snp$id`). We drop strand ambiguous variants (`drop_strand_ambig = TRUE`) by default. We could limit weights to protein coding genes (`filter_protein_coding_genes = TRUE`) based on the `gene_type` information in the "extra table" of PredictDB weights. We could set `ncore` to parallelize the computation.
