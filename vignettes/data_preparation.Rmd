...
for (i in 1:22) {
  if (!file.exists(file.path(outputdir, paste0(outname, "_chr", i, ".harmonized.z_snp.RDS")))){
    cat("Processing z-scores in chr", i, "\n")
    res <- preprocess_z_ld(z_snp = z_snp,
                           ...
                           gwas_n = gwas_n,
                           outputdir = outputdir,
                           outname = paste0(outname, "_chr", i),
                           detect_ld_mismatch = T, # If TRUE, detect LD mismatches using SuSiE RSS
                           flip_allele = T, # If TRUE, flip z-scores with allele flipping
                           ncore = ncore,
                           save_result = T)
  }
}
...
problematic_snps <- list()
for (i in 1:22){
  ld_mismatch_chr_res <- readRDS(file.path(outputdir, paste0(outname, "_chr", i, ".ld_mismatch_res.RDS")))
  problematic_snps[[i]] <- ld_mismatch_chr_res$problematic_snps
}
problematic_snps <- unlist(problematic_snps)
write.table(problematic_snps, file.path(outputdir, paste0(outname, ".problematic_snps.txt")), 
            col.names = F, row.names = F, quote = F)
```

If you do not want to detect LD mismatches (and allele flipping) using use SuSiE RSS, you can turn off `detect_ld_mismatch`. It runs much faster without detecting LD mismatches, 
so we can simply run all the chromosomes together. 
