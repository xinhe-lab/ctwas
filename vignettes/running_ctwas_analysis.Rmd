...
The `finemap_regions()` function performs fine-mapping for the regions in the `selected_region_data`.
...
We first expand the region of interest with all SNPs using the assembled `region_data`. We also need the number of causal signals (`L`) for this region. This is obtained from the output of the earlier `screen_regions()` function.
...
selected_region_L <- selected_region_L[region_id]
```
