...
+ `save_cor`, `cor_dir` and `force_compute_cor`: we need correlation matrices during fine-mapping and visualizing the results with LD.  If `save_cor = TRUE`, it saves the correlation matrices in the `cor_dir` directory for future access. These are large datasets, so they are not saved by default. If you have correlation matrices saved in `cor_dir` and `force_compute_cor = FALSE` (default option), then if you rerun fine-mapping, cTWAS will automatically check to see if correlation matrices are available in `cor_dir` and reuse them. You can set `force_compute_cor = TRUE` so that it always computes the correlation matrices. 
