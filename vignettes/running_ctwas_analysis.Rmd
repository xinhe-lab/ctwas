...
When `expand = TRUE` (default option), the selected regions will be automatically expanded with all SNPs for the final fine-mapping step. If the number of SNPs in a region is more than `maxSNP`, It will trim the SNPs in the region (according to the Z-scores).
...
*Note*: when screening and fine-mapping regions with LD, we need larger memory to load LD matrices and compute correlation matrices. So we recommend increasing memory or reducing the number of `ncore` for these steps.
