...
gene_annot <- get_gene_annot_from_ens_db(ens_db = ens_db, gene_ids)
```
...
When we have multiple contexts (e.g. tissues) or types (e.g. expression, splicing), it is useful to evaluate the combined PIP for each gene by contexts, types, or groups.
...
If we want to show the correlations with the focus gene, we need correlation matrices of the region. 
