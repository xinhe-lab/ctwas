...
```{r combine_pips}
# combine gene PIPs by contexts
combined_pip_by_context <- combine_gene_pips(finemap_res, 
                                             by = "context",
                                             filter_cs = TRUE,
                                             filter_protein_coding_genes = TRUE)

# combine gene PIPs by types
combined_pip_by_type <- combine_gene_pips(finemap_res, 
                                          by = "type",
                                          filter_cs = TRUE,
                                          filter_protein_coding_genes = TRUE)

# combine gene PIPs by groups
combined_pip_by_groups <- combine_gene_pips(finemap_res, 
                                            by = "group",
                                            filter_cs = TRUE,
                                            filter_protein_coding_genes = TRUE)

# select combined PIP > 0.8
sig_gene_pips_df <- combined_pip_by_groups[combined_pip_by_groups$combined_pip > 0.8, ]
