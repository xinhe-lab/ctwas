...
You may have multiple datasets with the same type and context, e.g. GTEx brain and CMC brain eQTLs. In this case, if you set `context = "brain"` and `type = "expression"` for both datasets, the two brain expression datasets would be merged into a single group. If you prefer to treat them as separate groups, you can give them different context names, e.g. `context = “GTEx_brain”` and `“CMC_brain”`. 
