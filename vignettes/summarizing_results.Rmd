...
+ `finemap_res`: fine-mapping results, a data frame of molecular traits and variants, including their IDs ("id"), the regions they belong to ("region_id"), Z-scores ("z"), PIPs ("susie_pip"), credible set indices ("cs_index"), and estimated effect size variance ("mu2"). 

The IDs of molecular traits ("id") used in cTWAS is in the format of 
<molecular_ id|weight_name>. "molecular_id" are the IDs originally used in the PredictDB or FUSION weights for genes or other molecular traits, such as Ensembl gene IDs. `weight_name` is the user defined weight name when preprocessing weights, see the tutorial ["Preparing cTWAS input data"][preparing_input_data_tutorial]). We can use the `get_molecular_ids()` function to extract the molecular IDs and add a column "molecular_id" to the fine-mapping results: 
...
We can also convert Z-scores to p-values, and add to the fine-mapping results. 
```{r add_pval}
finemap_res$p <- z2p(finemap_res$z)
head(finemap_res)
```
...
We focus on examples from running cTWAS with LD here, but the steps below also mostly work for running without LD, with one exception that we could not limit results to credible sets in the "no-LD" version of cTWAS results.
...
+ "combine_cs": it first sums PIPs of molecular traits of a genes in each credible set, and then combine PIPs using the following formula: $1 - \prod_k (1 - \text{PIP}_k)$, where $\text{PIP}_k$ is the summed PIP of the $k$-th credible set of a gene. This is the default option for combining PIPs from fine-mapping with LD. 
We also provide two other options:
+ "sum": sum over PIPs of all molecular traits for the same gene. We will use this option for for combining PIPs from fine-mapping without LD. 
+ "combine_all":  use the above formula for all molecular traits for the same gene, where $\text{PIP}_k$ is the PIP of the $k$-th molecular traits or contexts of a gene. 

For example, when using gene expression (eQTL) data from multiple tissues, we can use the function to compute combined PIPs across contexts, using the "combine_cs" method. By default, we limit the results to molecular traits within credible sets (`filter_cs = TRUE`) . 
...
```{r interpret_res_2}
subset(finemap_res, group != "SNP" & gene_type == "protein_coding" & susie_pip > 0.8 & cs_index != 0)
```
...
To map molecular traits to their corresponding genes, we need a data frame `mapping_table`, with IDs of molecular traits ("molecular_id"), and the corresponding gene names ("gene_name"). It is helpful to include additional gene information, such as gene types and gene positions. Below is an example for splicing trait: 

```{}
"
                 molecular_id gene_name      gene_type  chrom      start       end
 intron_1_151895194_151910954     THEM4 protein_coding      1  151895194 151910954
  intron_15_43038232_43043215      UBR1 protein_coding     15   43038232  43043215
  intron_15_43043395_43047161      UBR1 protein_coding     15   43043395  43047161
  intron_15_43047289_43048392      UBR1 protein_coding     15   43047289  43048392
"
```


Using this mapping table, we can use the `anno_finemap_res()` function to add additional columns to the fine-mapping results. If a molecular trait is mapped to multiple genes, it splits the PIP of that molecular trait to its target genes. 
...
```{r combined_pip_by_type, eval=FALSE}
...
```{r locus_plot_no_coloring, fig.width=10, fig.height=8}
...
The locus plot shows several tracks in the example region in chromosome 6. The top one shows -log10(p-value) of the association of variants (from GWAS) and molecular traits (from the package computed z-scores) with the phenotype. We limit results to protein coding genes by default. The data points are colored by the correlations with the focal gene (by default, it uses the gene with the highest PIP, HPR in this case). Users could specify the focal gene by setting the argument `focal_gene`, e.g. `focal_gene = HPR`; or specify the focal ID by setting `focal_id`. The next track shows the PIPs of variants and molecular traits. By default, we limit results to credible sets in the PIP track (`filter_cs = TRUE`), and color data points by the "cs_index" (`color_PIP_by_cs = TRUE`).  We can draw a red dotted line to show the PIP cutoff of 0.8 by setting `highlight_pip = 0.8`. The next track shows the QTLs of the focal gene. You can set `weights = NULL`, if you don't want to plot the QTL track. The bottom is the gene track.

The plotting function could color the molecular traits and SNPs based on the correlations among them. If we have saved correlation matrices in `cor_dir` when running cTWAS with LD, we could simply load those precomputed correlation matrices of the region using the function `load_region_cor()`. 
...
The locus plot shows the whole region by default. We could zoom in a region of interest by specifying the `locus_range` argument: 
...
In this plot, one can see that there are several genes with good associations with the phenotype (top panel), but only HPR in liver and HP in adipose are prioritized by cTWAS (second panel). This is because once HPR, HP and other likely causal variants are selected by cTWAS, there is no remaining association left in other molecular traits. This means that other associations are likely due to their correlations with the prioritized genes rather than representing independent signals.

For the "no-LD" version, we could still make the locus plot. The difference is that it could not color the data points based on the correlations to the focal gene, and could not limit results in the PIP track to credible sets.
