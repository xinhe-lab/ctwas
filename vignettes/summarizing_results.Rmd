...
We can view the molecular traits above a certain PIP cutoff, e.g. PIP > 0.8 and limit to credible sets, by: 
...
We can drop unannotated genes by setting `drop_unannotated_genes = TRUE`,  limit to protein coding genes by setting `filter_protein_coding_genes = TRUE`, and limit to credible sets by setting `filter_cs = TRUE`. 
