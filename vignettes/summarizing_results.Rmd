...
+ `screened_region_data`: the data of all SNPs and genes of the selected regions. 
...
These plots show the estimated prior inclusion probability, prior effect size variance, and enrichment over the iterations of parameter estimation, if cTWAS is run properly. The enrichment is defined as the ratio of the prior of genes over the prior of variants. We generally expect genes to have higher prior inclusion probability than variants. Enrichment values typically range from 20 -100 for eQTLs. 

Then, we use `summarize_param()` to obtain estimated parameters (from the last iteration) and to compute the proportion of variance explained (PVE) by variants and genes. These PVE values allow us to compute how heritability of the phenotype is partitioned among variants and groups of genes. 
...
To interpret and visualize the results, we first need to add gene annotations to the fine-mapping result.

We use the `get_gene_annot_from_ens_db()` function to obtain the gene annotations from the Ensembl database:
...
We list genes with PIP > 0.8, which is the threshold we used in the paper. 

```{r interpret_res}
subset(finemap_res, group == "gene" & susie_pip > 0.8)
```


In this example, HPR gene is the only gene that satisfies this threshold.
...
The locus plot shows several tracks in the example region in chromosome 6 (71.6 - 72.4 Mb). The top one shows -log10(p-value) of the association of variants and genes with the phenotype. The next track shows the PIPs of variants and genes.  The data points are colored by the correlations with the focus gene (HPR in this locus). The red dotted line shows the PIP cutoff of 0.8. The next track shows the locations of QTLs of the focus gene (by default, the focus gene is the gene with the highest PIP). The bottom is the gene track.
