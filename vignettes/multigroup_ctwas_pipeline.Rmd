...
Here, we perform the LD mismatch diagnosis using [SuSiE-RSS][susierss_diagnostic] for selected regions with high PIP signals. 

We first select regions with total non-SNP PIP > 0.5. 
```{r, eval=FALSE}
...
We perform LD mismatch diagnosis for these regions using SuSiE RSS.
```{r, eval=FALSE}
...
We then rerun the fine-mapping with L = 1 for the regions containing high PIP SNPs 
detected with LD mismatches, or high PIP molecular traits with problematic SNPs in their weights. 
```{r, eval=FALSE}
# select regions containing molecular traits with problematic SNPs in their weights
problematic_region_ids <- select_problematic_regions(problematic_snps, weights, z_gene, z_cutoff)
