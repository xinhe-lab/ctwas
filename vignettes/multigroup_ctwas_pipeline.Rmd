...
We need to specify the GWAS summary statistics (`z_snp`), results from computing gene z-scores (including `z_gene`, `gene_info`), 
and `region_info`. 
...
finemap_region(z_snp, z_gene, region_info, gene_info, wgtlist, regionlist, region_tag, 
               L = 5, group_prior, group_prior_var, save_cor, cor_dir, ...)
```
...
```{r, eval=FALSE}
rerun_region_tags <- get_problematic_regions(ctwas_res, weight, problematic_snps, pip_thresh = 0.5)
finemap_regions_L1(z_snp, z_gene, gene_info, regionlist, rerun_region_tags, weight_list, group_prior, group_prior_var, ...)
```
