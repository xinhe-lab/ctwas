...
setwd("/project2/xinhe/shared_data/multigroup_ctwas/LDL_multigroup_tutorial/")
...
Please check [Fusion/TWAS](http://gusevlab.org/projects/fusion/#compute-your-own-predictive-models) for the format of FUSION weights. To specify weights in FUSION format, provide the path to the folder that contains the `.wgt.RDat` files. The `.pos` file which points to the individual *.RDat weight files is assumed to be outside the folder. For more format details, check the FUSION website. 
...
In this example, we use three weights, one eQTL model, one sQTL model and one 3‘aQTL model from liver. eQTL and sQTL model are downloaded from [PredictDB website](https://predictdb.org/post/2021/07/21/gtex-v8-models-on-eqtl-and-sqtl/). 3‘aQTL model (Alternative polyadenylation) is in FUSION format and downloaded from [TCGD database](http://bioinfo.szbl.ac.cn/TCGD/download.php). 
...
weight_files <- c("/project2/xinhe/shared_data/multigroup_ctwas/weights/expression_models/mashr_Liver.db",
                  "/project2/xinhe/shared_data/multigroup_ctwas/weights/splicing_models/mashr_Liver.db",
                  "/project2/xinhe/shared_data/multigroup_ctwas/weights/apa_models/Liver/Liver/")
ncore <- 6
...
weights_liver_apa <- preprocess_weights(weight_file = weight_files[3],
                                        region_info,
                                        z_snp$id,
                                        type = "aQTL",
                                        context = "Liver",
...
weights <- c(weights_liver_expression, weights_liver_splicing, weights_liver_apa)
```
...
See [the tutorial "Running cTWAS modules separately"](https://xinhe-lab.github.io/multigroup_ctwas/articles/multigroup_ctwas_modules.html) for more details.

## Postprocessing

### Merging regions

For cross-boundary genes, we will merge the regions for high PIP (>0.5) genes. 
...
```{r, eval=FALSE}
# finemap_res <- update_merged_regions_finemap_res(finemap_res, finemap_merged_regions_res, merged_region_data)
```
...
```{r, eval=FALSE}
high_PIP_region_info <- region_info[region_info$region_id %in% highPIP_region_ids, , drop=FALSE]
res <- detect_ld_mismatch_susie(z_snp, high_PIP_region_info, gwas_n, ncore = ncore, p_diff_thresh = 5e-8)
...
finemap_merged_regions_res <- finemap_regions(rerun_region_data,
                                              region_info,
                                              weights,
                                              group_prior = group_prior,
                                              group_prior_var = group_prior_var,
...
as well as the z-scores for each SNP and gene. 
