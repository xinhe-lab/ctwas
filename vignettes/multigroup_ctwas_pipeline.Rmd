...
The `max_snp_region` sets a maximum on the number of variants that can be in a single region to prevent memory issues during fine-mapping. 
...
finemap_res <- update_finemap_res(finemap_res, finemap_problematic_regions_res)
```
