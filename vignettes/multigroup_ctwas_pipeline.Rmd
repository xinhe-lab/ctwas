...
In this example, we use two weights, one eQTL model, one sQTL model from the liver. eQTL and sQTL models are downloaded from [PredictDB website](https://predictdb.org/post/2021/07/21/gtex-v8-models-on-eqtl-and-sqtl/). 
...
weight_files <- c("/project2/xinhe/shared_data/multigroup_ctwas/weights/expression_models/mashr_Liver.db","/project2/xinhe/shared_data/multigroup_ctwas/weights/splicing_models/mashr_Liver.db")
...
weights_liver_splicing <- preprocess_weights(weight_file = weight_files[2],
                                             region_info,
                                             z_snp$id,
                                             type = "sQTL",
                                             context = "liver",
...
weights <- c(weights_liver_expression, weights_liver_splicing)
```
...
TODO: add details about the input, output and parameters
...
This function computes z-scores for the molecular traits using preprocessed SNP z-scores (`z_snp`) and the preprocessed weights. 
...
We use the `est_param` function to estimate parameters. It will take the assembled `region_data` as input. 
...
This step will run a screening process to fine-map all regions with thinned SNPs,
...
We now perform the fine-mapping step with the estimated parameters for each of the screened regions.
...
We could run finemapping for a single region or a few regions with precomputed parameters.
...
If  we don't have pre-assembled region data, we can also assemble `region_data` for this region with full SNPs (set `thin=1`).
But one caveat, we could not adjust/reassign boundary genes in this way, as we have only one region here. 
...
We select cross-boundary genes with PIP >= 0.5, then identify overlapping regions from these high PIP cross-boundary genes, and merge the region_data for these regions. 
...
if (nrow(high_PIP_boundary_genes) > 0){
  ...
  rm(res)
  # Merge region_data for the overlapping regions
  merged_region_data <- merge_region_data(merged_region_list,
                                          region_data,
                                          region_info,
                                          z_snp,
                                          z_gene,
...
The returned `merged_region_data` object only contains data for merged regions.
...
Update the finemapping results for those merged regions.
```{r, eval=FALSE}
# finemap_res <- update_finemap_res(finemap_res, finemap_merged_regions_res, merged_region_data)
```


### Rerun finemapping with L = 1 for regions with LD mismatches

Select regions with high PIP (>0.5) SNPs and molecular traits.
```{r, eval=FALSE}
highPIP_finemap_res <- finemap_res[finemap_res$susie_pip > 0.5, ]
highPIP_region_ids <- unique(highPIP_finemap_res$region_id)
```


We use SuSiE RSS to perform LD mismatch diagnosis for these high PIP regions.
```{r, eval=FALSE}
highPIP_region_info <- region_info[region_info$region_id %in% highPIP_region_ids,]
res <- detect_ld_mismatch_susie(z_snp, highPIP_region_info, gwas_n, ncore = ncore, p_diff_thresh = 5e-8)
problematic_snps <- res$problematic_snps
```


We will rerun the fine-mapping with L = 1 for the regions containing high PIP SNPs 
detected with LD mismatches, or high PIP molecular traits with problematic SNPs in their weights. 
```{r, eval=FALSE}
# select regions containing high PIP (>0.5) SNPs detected with LD mismatches, 
# or high PIP molecular traits with problematic SNPs in their weights
problematic_region_ids <- select_problematic_regions(problematic_snps, highPIP_finemap_res, weights)
rerun_region_data <- expand_region_data(region_data[problematic_region_ids],
                                        region_info,
                                        z_snp,
                                        z_gene,
                                        trim_by = "z",
...
# select PIP > 0.8
finemap_res <- finemap_res[order(-finemap_res$susie_pip),]
ctwas_res <- finemap_res[finemap_res$type!="SNP" & finemap_res$susie_pip > 0.8,]
```
