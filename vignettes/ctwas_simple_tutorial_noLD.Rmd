...
We require a data frame `region_info` containing the region definitions, with the following columns: "chrom", "start", "stop", for the genomic coordinates of the regions.
...
We will need to have a list of variants as a “reference”, with the position and allele information of these variants. We provide a function, `preprocess_region_LD_snp_info()` to preprocess region definitions and map variants from the LD reference to regions, which takes input of `region_info` and a data frame of variant information from the entire genome, and returns processed `region_info`, `snp_info`.
...
Please see the [tutorials]() for running cTWAS using the LD version. 
...
```{r ctwas_sumstats_noLD, eval=FALSE, message=FALSE}
...
```{r, include=FALSE}
ctwas_res <- readRDS(system.file("extdata/sample_data", "ctwas_sumstats_noLD_res.RDS", package = "ctwas"))
```
