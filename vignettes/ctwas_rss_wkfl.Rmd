...
```{r input}
zdf.SNP <- fread("/home/simingz/causalTWAS/simulations/simulation_ashtest_20201001/20201001-1-1.snpgwas.txt.gz", header =T)
setnames(zdf.SNP, old = c('MARKER_ID', 't.value',), new = c('id', 'z'))
ld_pgenfs <- paste0("/project2/mstephens/causalTWAS/ukbiobank/ukb_pgen_s40.22/ukb_chr", 1:22, "_s40.22.pgen")
...
for (i in 1:22){
  pgenf <- pgenfs[i]
  res <- impute_expr_z(zdf.SNP, pgenf = pgenf, weight = weight,
...
```{r ctwas}
outputdir <- "/project2/mstephens/causalTWAS/simulations/test_package_temp"
exprfs <- paste0("/project2/mstephens/causalTWAS/simulations/test_package_temp/z_chr", 1:22, ".expr.gz")
ctwas_rss(zdf, pgenfs, exprfs, ld_regions = "EUR", thin = 0.1, outputdir = outputdir, outname = "z", ncore = 10)
```
