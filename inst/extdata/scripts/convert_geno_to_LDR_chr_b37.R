...
setwd("/gpfs/data/xhe-lab/ukb_LDR/matrices_0.1_b37")
...
# specify output
out_filestem <- "ukb_b37_0.1_chr"
out_dir <- "/scratch/wcrouse/LDR"

####################

dir.create(out_dir, showWarnings=F)
if (is.null(ld_regions_custom)) {
  regionfile <- system.file("extdata", "ldetect", paste0(ld_regions, ".b37.bed"), package = "ctwas")
} else {
...
#load build 37 positions
snpinfo_hg37_all <- fread("/gpfs/data/xhe-lab/ukb_LDR/neale_lab/neale_variants.bim")
colnames(snpinfo_hg37_all) <- c("chr", "id", "cm", "pos", "alt", "ref")
...
#for (b in 1:length(ld_pvarfs)) {
{
  ...
  snpinfo_hg37 <- snpinfo_hg37_all[snpinfo_hg37_all$chr==b,]
  rm(snpinfo_hg37_all)###for single chromosome only
  ...
  for (rn in reg_start:nrow(regions)) {
    ...
    if (!(file.exists(outfile_Rvar)| file.exists(outfile_temp))){
      ...
      sidx_hg37 <- which(snpinfo_hg37$pos >= p0 & snpinfo_hg37$pos < p1)
      sid_hg37 <- snpinfo_hg37$id[sidx_hg37]
      
      sidx <- which(snpinfo$id %in% sid_hg37)
      sid <- snpinfo[sidx, "id"]
      ...
      R_snp_variances <- R_snp_variances[sid_hg37]
      names(R_snp_variances) <- NULL

      R_snp_AFs <- colSums(X.g)/(2*nrow(X.g))
      names(R_snp_AFs) <- sid$id
      R_snp_AFs <- R_snp_AFs[sid_hg37]
      names(R_snp_AFs) <- NULL
    
      R_var <- cbind(snpinfo_hg37[sidx_hg37,c("chr","id","pos","alt","ref")], R_snp_variances, R_snp_AFs)
