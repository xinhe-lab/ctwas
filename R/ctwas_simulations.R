...
read_weight_predictdb <- function (weight,
  ...
  outlist <- foreach::foreach(core = 1:ncore, .combine = "c", .packages = "ctwas") %dopar% {
...
read_exprvar <- function(exprvarf){
  ...
  exprvar <- try(fread(exprvarf, header = T))
...
read_expr <- function(exprf, variantidx = NULL){
  if (!is.null(variantidx) & length(variantidx)==0){
    ...
    return(as.matrix(fread(exprf, header = F, select = variantidx)))
  }
}
