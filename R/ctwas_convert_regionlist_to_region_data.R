...
convert_regionlist_to_region_data <- function(regionlist,
  ...
  # convert the data structure from old version regionlist to new version region data
  logging::loginfo("Convert the data structure from regionlist to region data")
  region_data <- list()
  for(b in 1:22){
    ...
    for(rn in names(regionlist_chr)){
      ...
      if(regiondata$start != regioninfo$start || regiondata$stop != regioninfo$stop){
        stop(paste("region does not match for chr:", b, "rn:", rn))
      }
      regiondata$chrom <- b
      region_id <- paste0(regiondata$chrom, ":", regiondata$start, "-", regiondata$stop)
      regiondata$region_id <- region_id
      regiondata$thin <- thin
      regiondata <- regiondata[c("region_id", "chrom", "start", "stop", "minpos", "maxpos", "gid", "sid", "thin")]
      region_data[[region_id]] <- regiondata
    }
  }
  logging::loginfo("%d regions in region_data", length(region_data))

  # add z-scores to region_data
  logging::loginfo("Add z-scores to region_data...")
