...
#'
#' @param point.sizes size values for SNP and gene data points in the scatter plots
#'
#' @param point.alpha alpha values for SNP and gene data points in the scatter plots
#'
...
make_locusplot <- function(finemap_res,
  ...
  if (!is.null(focal_gene)) {
    ...
    finemap_region_gene_res <- finemap_region_res[finemap_region_res$group == "gene",]
  ...
  if (!is.null(R_gene) && !is.null(R_snp_gene)) {
    ...
    finemap_region_res$r2[finemap_region_res$group=="gene"] <- R_gene[finemap_region_res$id[finemap_region_res$group=="gene"], focal_gid]^2
    finemap_region_res$r2[finemap_region_res$group=="SNP"] <- R_snp_gene[finemap_region_res$id[finemap_region_res$group=="SNP"], focal_gid]^2
  ...
  loginfo("gene: %s", focal_gene_name)
  ...
  p_qtl <- ggplot(finemap_qtl_res, aes(x=.data$pos/1e6)) +
    ...
    labs(title = focal_gene_name, y = "QTL") +
    theme(
