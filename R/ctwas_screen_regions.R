...
screen_regions_noLD <- function(region_data,
  ...
  screened_region_ids2 <- screen_summary$region_id[which(screen_summary$nonSNP_PIP >= min_nonSNP_PIP)]

  setequal(screened_region_ids, screened_region_ids2)
...
#' @title Screens regions with strong signals using LD.
#'
#' @param region_data a list object indexing regions, variants and genes.
#'
...
#' @export
#'
screen_regions_LD <- function(region_data,
                              LD_map,
                              weights,
                              group_prior = NULL,
                              group_prior_var = NULL,
  ...
  if (filter_nonSNP_PIP) {
    ...
    if (L > 1){
      ...
      finemap_screening_res <- finemap_regions_ser(screened_region_data,
                                                   group_prior = group_prior,
                                                   group_prior_var = group_prior_var,
                                                   null_method = null_method,
                                                   null_weight = null_weight,
