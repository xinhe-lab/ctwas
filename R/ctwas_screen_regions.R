...
#' @title Screens regions with strong signals without using LD (L=1).
#'
#' @param region_data a list object indexing regions, variants and genes.
#'
#' @param group_prior a vector of two prior inclusion probabilities for SNPs and genes.
...
screen_regions_noLD <- function(region_data,
  ...
  # run finemapping for all regions without LD (L=1) using estimated priors
  # select regions with total non-SNP PIPs > 0.5
  loginfo("Computing non-SNP PIPs ...")
...
#' @title Screens regions with strong signals with SER model
#'
#' @param region_data a list object indexing regions, variants and genes.
#'
#' @param group_prior a vector of two prior inclusion probabilities for SNPs and genes.
...
#' @export
#'
screen_regions_ser <- function(region_data,
                               group_prior = NULL,
                               group_prior_var = NULL,
                               min_var = 2,
                               min_gene = 1,
  ...
  loginfo("Screening regions with SER model ...")
  null_method <- match.arg(null_method)
  ...
  # select regions with total non-SNP PIPs > 0.5
  loginfo("Computing non-SNP PIPs ...")
  finemap_screening_res <- finemap_regions_ser(region_data,
                                               group_prior = group_prior,
                                               group_prior_var = group_prior_var,
                                               null_method = null_method,
                                               null_weight = null_weight,
