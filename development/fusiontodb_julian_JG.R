# A function to convert fusion weights to predictDB format
# This function was adapted from Julian's code.
...
# input files: 
# template_db_path: path to a template PrediXcanDB file
# rdata_dir: directory path that stores fusion weights which end with .wgt.RDat
# out_db_path: path to the output
# expect_hg38_pos_in_snps: whether expect SNP position to be in hg38 
## `varID` column would take different formats according to the genome build
# want_model_type: select one prediction model from the available ones
## For top1 model, the single best eQTL with the largest magnitude will be selected.

fusion_to_predictDB <- function(
    template_db_path = "template_predixcan.db",
    rdata_dir = "fusion_weights",
    out_db_path = "test.db",
